...
<div id="m1" class="modal fade" role="dialog">
  <div class="modal-dialog" id="set9">
    ...
    <div class="modal-header text-center"><h1 id="t3">Add Jadwal</h1></div>
	  ...
	  $cmd=mysqli_query($con,"insert into tbl_jadwal_event (id_event,jadwal_pertandingan,lokasi_pertandingan,live_pertandingan,tim_a,tim_b,score_a,score_b) values 
	  ('".$_GET["id_event"]."','".$_POST["jadwal_pertandingan"]."','".$_POST["lokasi_pertandingan"]."','".$_POST["live_pertandingan"]."','$tim_a','$tim_b','".$_POST["score_a"]."','".$_POST["score_b"]."')");      
	  header("refresh:0");
      //echo "insert into tbl_jadwal_event (id_event,jadwal_pertandingan,lokasi_pertandingan,live_pertandingan,tim_a,tim_b,score_a,score_b) values ('".$_GET["id_event"]."','".$_POST["jadwal_pertandingan"]."','$lokasi_pertandingan','$live_pertandingan',$tim_a','$tim_b','".$_POST["score_a"]."','".$_POST["score_b"]."')";
	  //exit;
      }
      ?>	
	  <hr />
...
<div class="form-group text-left" id="t1">Or <br /> <br />Add Tim B <input type="text" name="tim_b_name" class="form-control" id="ipt1">
  ...
  <hr />
    ...
    <div class="form-group text-left" id="t1">Stadion<input type="text" name="lokasi_pertandingan" class="form-control" id="ipt1">
	</div>	
	<div class="form-group text-left" id="t1">Live TV<input type="text" name="live_pertandingan" class="form-control" id="ipt1">
	</div>	
...
$result=mysqli_query($con,"SELECT a.*,c.logo as logo_a,d.logo as logo_b,c.name as club_a,d.name as club_b FROM tbl_jadwal_event a INNER JOIN tbl_event b ON b.id_event=a.id_event INNER JOIN tbl_club c ON c.club_id=a.tim_a INNER JOIN tbl_club d ON d.club_id=a.tim_b where a.id_event='".$_GET["id_event"]."' order by jadwal_pertandingan desc");
...
<td>'.$data["lokasi_pertandingan"].'</td>
<td>'.$data["live_pertandingan"].'</td>
