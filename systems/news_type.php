...
<div id="m1" class="modal fade" role="dialog">
  <div class="modal-dialog" id="set9">
    ...
    <div class="modal-header text-center"><h1 id="t3">Add News Type</h1></div>
	  ...
	  $cmd=mysqli_query($con,"insert into tbl_news_types (news_type) values ('$news_type')");      
           header("refresh:0");
      }	
      ?>	
	  <div class="form-group text-left" id="t1">News Type Name<input type="text" name="news_type" class="form-control" id="ipt1" required></div>
...
$result=mysqli_query($con,"SELECT a.*,b.fullname FROM tbl_news_types a LEFT JOIN tbl_admin b ON b.admin_id=a.admin_id where news_type_id order by news_type_id desc");
...
print'<tr>
<td>'.$news_type.'</td>
<td><a href="newstype_edit?admin_id='.$admin_id.'&news_type_id='.$news_type_id.'" class="btn" id="btn3">EDIT</a>&emsp;<a href="newstype_delete?admin_id='.$admin_id.'&news_type_id='.$news_type_id.'" class="btn" id="btn4">DELETE</a></td>
</tr>';
}
