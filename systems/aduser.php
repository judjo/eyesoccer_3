...
<h1 id="t2"><i class="fa fa-newspaper-o"></i> LIST ADMIN</h1>
<hr></hr>
...
<div id="m1" class="modal fade" role="dialog">
  <div class="modal-dialog" id="set9">
    ...
    <div class="modal-header text-center"><h1 id="t3">Add ADMIN</h1></div>
    ...
    $cmd=mysqli_query($con,"insert into tbl_admin (admin_id,title,pic,thumb1,note) values ('".$_SESSION["admin_id"]."','$title','$pic','$thumb1','$note')");      
           header("refresh:0");
      } 
      }
      ?>  
...
$result=mysqli_query($con,"SELECT * FROM tbl_admin where admin_id order by admin_id desc");
...
<td>'.$username.'</td>
<td>'.$fullname.'</td>
<td><a href="aduser_edit?admin_id='.$admin_id.'&ads_id='.$ads_id.'" class="btn" id="btn3">EDIT</a>&emsp;<a href="aduser_delete?admin_id='.$admin_id.'&ads_id='.$ads_id.'" class="btn" id="btn4" onclick=\'confirm("Apa anda yakin untuk menghapus ?")\'>DELETE</a></td>
</tr>';
}
