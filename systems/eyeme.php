...
<div id="m1" class="modal fade" role="dialog">
  <div class="modal-dialog" id="set9">
    ...
    <div class="modal-header text-center"><h1 id="t3">Add</h1></div>
...
<td>'.$title.'</td>
<td>'.$news_type.'</td>
<td>'.$data["upload_name"].'</td><td>'.$data["publish_by"].'</td>
<td><a href="eyeme_view?admin_id='.$admin_id.'&id_gallery='.$id_gallery.'" class="btn" id="btn3">View</a>  <a href="eyeme_delete?admin_id='.$admin_id.'&id_gallery='.$id_gallery.'" class="btn" id="btn4" onclick=\'confirm("Apa anda yakin untuk menghapus ?")\'>DELETE</a></td>
</tr>';
}
...
<script type="text/javascript">
	...
	function myFunction() {
		total_me=$(".total_me").val();
		$.ajax({
			url: "view_notification_me.php",
			type: "POST",
			data: { 'total_me': total_me},  
