...
//$ads_id=$_GET['ads_id'];
$cmd=mysqli_query($con,"select * from tbl_admin where admin_id='$admin_id'");
...
<h1 id="t2"><i class="fa fa-newspaper-o"></i> UPDATE ADS</h1>
<hr></hr> 
<form method="post" enctype="multipart/form-data">

<div class="form-group">Username<input type="text" name="username" value="<?php print $row['username']; ?>" class="form-control" id="ipt1" required></div>
<div class="form-group">Fullname<input type="text" name="fullname" value="<?php print $row['fullname']; ?>" class="form-control" id="ipt1" required></div>
...
if(empty($pic)){
$cmd=mysqli_query($con,"update tbl_admin set username='$username',fullname='$fullname' where admin_id='$admin_id'");
//$cmd=mysqli_query($con,"update tbl_ads set title='$title',category_name_ads='$category_name_ads' where ads_id='$ads_id'");

header("location:aduser?admin_id=$admin_id");  
}
else{
