...
<div id="m1" class="modal fade" role="dialog">
  <div class="modal-dialog" id="set9">
    ...
    <div class="modal-header text-center"><h1 id="t3">Add Klasment</h1></div>
	  ...
	  if(mysqli_num_rows(mysqli_query($con,"select * from tbl_clasment"))>14){
    ...
    $cmd=mysqli_query($con,"insert into tbl_clasment (admin_id,title,pic,thumb1) values ('".$_SESSION["admin_id"]."','$title','$pic','$thumb1')");      
           header("refresh:0");
      } 
      }
      ?>  
...
$result=mysqli_query($con,"SELECT a.*,b.fullname FROM tbl_clasment a LEFT JOIN tbl_admin b ON b.admin_id=a.admin_id where clasment_id order by clasment_id desc");
while($data = mysqli_fetch_array($result))
{
$clasment_id=$data['clasment_id'];  
$title=$data['title'];
//$note=$data['note'];
$pic=$data['pic'];
print'<tr>
...
<td>'.$title.'</td>
<td><a href="classment_edit?admin_id='.$admin_id.'&clasment_id='.$clasment_id.'" class="btn" id="btn3">EDIT</a>&emsp;<a href="clasment_delete?admin_id='.$admin_id.'&clasment_id='.$clasment_id.'" class="btn" id="btn4" onclick=\'confirm("Apa anda yakin untuk menghapus ?")\'>DELETE</a></td>
</tr>';
}
