...
$ads_id=$_GET['ads_id'];
$cmd=mysqli_query($con,"select * from tbl_ads where ads_id='$ads_id'");
...
<?php
$cmd1=mysqli_query($con,"select * from tbl_category_ads");
while($row1=mysqli_fetch_array($cmd1)){
print '<option'; if($row['note']==$row1['note']){print " selected";}else{print "";} print'>'.$row1['note'].'</option>';  
}
?>  
</select> 
</div>
...
if(empty($pic)){
$cmd=mysqli_query($con,"update tbl_ads set title='$title',note='$note' where ads_id='$ads_id'");
