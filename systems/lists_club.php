...
<h1 id="t2"><i class="fa fa-futbol-o fa-lg"></i> CLUB</h1>
<hr></hr>
...
$x++;
if($x==1)
$construct .="a.name LIKE '%$search_each%'";
else
$construct .="AND a.name LIKE '%$search_each%'";
    
}
  
$constructs = mysqli_query($con,"SELECT * FROM tbl_club WHERE $construct");
...
if(!$start)
$start=0; 
$getquery = mysqli_query($con,"SELECT a.*,b.username FROM tbl_club a LEFT JOIN tbl_users b ON b.user_id =a.user_id WHERE $construct LIMIT $start, $per_page");

while($runrows = mysqli_fetch_assoc($getquery))
{
$club_id=$runrows['club_id']; 
$name=$runrows['name']; 
$establish_date=$runrows['establish_date'];
$logo=$runrows['logo'];
print'
...
<td>'.$name.' <br /><small>'.$runrows["username"].'</small></td>
<td>'.$establish_date.'</td>
<td><a href="club_player?admin_id='.$admin_id.'&club_id='.$club_id.'" class="btn" id="btn3">EDIT</a>&emsp;<a href="club_player?admin_id='.$admin_id.'&club_id='.$club_id.'" class="btn" id="btn4">DELETE</a></td>
</tr>
</body>
</table>
</div>
...
{   
if (!($start<=0)) 
echo "<li><a href='lists_club?search=$search&submit=Search+source+code&start=$prev&admin_id=$admin_id' id='pg1'>Prev</a></li>";    
...
{
if ($i == $start){
echo "<li class='active'><a href='lists_club?search=$search&submit=Search+source+code&start=$i&admin_id=$admin_id' id='pg2'><b>$counter</b></a></li> ";
}
else {
echo "<li><a href='lists_club?search=$search&submit=Search+source+code&start=$i&admin_id=$admin_id' id='pg1'>$counter</a></li> ";
}  
$i = $i + $per_page;                 
}
}
...
{
echo "<li><a href='lists_club?search=$search&submit=Search+source+code&start=0&admin_id=$admin_id' id='pg1'>1</a></li>";
echo "<li><a href='lists_club?search=$search&submit=Search+source+code&start=$per_page&admin_id=$admin_id' id='pg1'>2</a> ....</li>";
...
echo "<li><a href='lists_club?search=$search&submit=Search+source+code&start=$next&admin_id=$admin_id' id='pg1'>Next</a></li>";    
}   
echo "</ul></div>";
} 
} 
