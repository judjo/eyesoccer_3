...
?>
<div id="official_reg" class="modal fade" role="dialog">
  <div class="modal-dialog" id="set9">
    <div class="modal-content" id="set8">
    <div class="modal-header text-center"><h1 id="t3">TAMBAH OFFICIAL</h1></div>
...
<div id="back3"><i class="fa fa-user"></i> Pendaftaran Official</div><br>
<div class="form-group"><a data-toggle="modal" data-target="#official_reg" class="btn" id="btn1"> <i class="fa fa-plus"></i> TAMBAH OFFICIAL </a></div>
...
$result=$this->db->query("SELECT a.* FROM tbl_official_team a INNER JOIN tbl_club b ON b.club_id=a.club_now INNER JOIN tbl_users c ON c.user_id=b.user_id where c.user_id='".$_SESSION["user_id"]."' order by a.name asc LIMIT ".$offset.", ".$dataPerPage);
foreach($result->result_array() as $data)
{
$player_id=$data['official_id'];	
$name=$data['name'];
...
if ($noPage > 1) echo  "<a href='official/".($noPage-1)."/".$club_id."' id='pg1'>Prev</a>&nbsp;";
...
else echo "<a href='official?page=$page&club_id=$club_id' id='pg1'>".$page."</a>&nbsp;";
$page = $page;          
}
}
if ($noPage < $jumPage) echo "<a href='official?page=".($noPage+1)."&club_id=$club_id' id='pg1'>Next</a>&nbsp;";
echo "</ul></center>";   
?>	
</div>

<div class="form-group text-right"><a href="<?=base_url()?>eyeprofile/daftar/player" class="btn" id="btn1">BERIKUTNYA</a></div>
