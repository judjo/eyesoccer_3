...
<div class="row">
<div class="col-md-2"><select class="form-control" id="set8">
<option>Sort By</option><option>Termurah</option><option>Termahal</option></select><div class="hidden-lg hidden-md"><br></div></div>
<div class="col-md-2">
<select name="category_product_name" class="form-control" id="set8">
<option value="">Pilih Kategori</option>
<?php
$product=$this->db->query("SELECT * FROM tbl_category_product ORDER BY category_product_name DESC");
foreach($product->result_array() as $pro)
{
	echo "<option value='".$pro["id_category_product"]."'>".$pro["category_product_name"]."</option>";
}
?>
</select></div>
...
<div class="form-group hidden-md hidden-lg"><br></div>
<hr></hr>
		<?php
			$cmd1=$this->db->query("select * from tbl_product where id_product order by id_product desc LIMIT 4")->result_array();
