...
<?php
		$cmd20=$this->db->query("SELECT * FROM tbl_eyetube where category_name in ('Eyesoccer Profile') order by eyetube_id desc");
		...
		<div class="row">		
		  <div class="media">
			...
			<div class="media-body">
			  <a href="'.base_url().'eyetube/detail/'.$eyetube_id.'" id="a4"><p class="media-heading" style="font-size:12px;">'.substr($row20["title"],0,25).'...</p>
      <small id="set6"><i class="fa fa-clock-o"></i> '.$row20['createon'].'</small></a><br>
		<small id="set6" style="color:#6A5ACD">'.$row20['tube_view'].' views </small> - <small style="color:#6A5ACD">'.$row20['tube_like'].' Suka</small>	  
			</div>
		  </div>
		  </div>
		  </div>
